 ...
 * 单文件 bundle（IIFE），无依赖（无 import 相对模块）
 * EdgeEver 加载此文件后从此全局变量读取：globalThis.EdgeEverCalendarPlugin
 * ============================================================ */
...
;(function () {
  ...
  const PLUGIN = {
    activate(context) {
      ...
      // 返回卸载函数
      return () => {
        try { unregisterPanel?.(); } catch (_) {}
        try { unregisterCmd?.(); } catch (_) {}
      };
    },
  };

  // 挂到全局（EdgeEver 通过这个变量读取插件入口）
  globalThis.EdgeEverCalendarPlugin = PLUGIN;
    ...
    async function refresh() {
      ...
      if (!state.notesCache.has(cacheKey)) {
        try {
          ...
          try { context.ui.showNotice('拉取笔记失败：' + (e?.message || e)); } catch (_) {}
        }
      }
      state.loading = false;
      ...
      detail.querySelectorAll('.zcc-note').forEach((el) => {
        el.addEventListener('click', (ev) => {
          ...
          context.ui.openNote(id).catch((e) => {
            ...
            try { context.ui.showNotice('打开笔记失败：' + (e?.message || e)); } catch (_) {}
          });
        });
      });
    }
  ...
  async function fetchMonthNotes(context, year, month) {
    ...
    while (true) {
      let page;
      try {
        page = await context.notes.query({
          sort: 'updated-desc',
          limit: pageSize,
          offset,
        });
